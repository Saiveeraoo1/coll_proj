...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    $qry = $conn->query("SELECT * FROM `visit_list` WHERE id = '$id'");
    if($qry->num_rows > 0){
    ...
    <form action="" id="visit-form">
        ...
        <div class="form-group">
            ...
            <select name="inmate_id" id="inmate_id" class="form-control form-control-sm rounded-0" required="required">
                <option value="" <?php echo !isset($inmate_id) ? 'selected' : ''; ?>></option>
                <?php 
                ...
                ?>
                <option value="<?php echo $row['id']; ?>" <?php echo $row['visiting_privilege'] == 0 ? 'disabled' : ''; ?> <?php echo isset($inmate_id) && $inmate_id == $row['id'] ? 'selected' : ''; ?>>Inmate-<?php echo $row['code']; ?> <?php echo $row['name']; ?> <?php echo $row['visiting_privilege'] == 0 ? '(Disallowed)' : ''; ?></option>
                <?php endwhile; ?>
            </select>
        </div>
        <div class="form-group">
